...
class ListTimeTest extends TestCase
{
    ...
    /** @test */
    public function only_an_user_can_access_the_times_calendar_page()
    {
        $user = User::factory()->user()->create();
        $organization = Organization::factory()->create(['owner_id' => $user->id]);
        $user->update(['organization_id' => $organization->id]);

        $this->get('/time/calendar')->assertRedirect('/login');

        $this->actingAs($user);

        $this->get('/time/calendar')->assertRedirect(route(homeRoute()));

        $user->syncPermissions([
            Permission::where('name', 'user.access.times.access')->first()->id, 
        ]);

        $this->get('/time/calendar')->assertOk();
    }
}
